...
diff results/deepstarr/5dreamrnn/1deepstarr/100random_proposal/100random_acquisition/100000cand_20000acq/init_prop_genomic_acq_random_20k/val_genomic/idx0/summary.json \
     results/deepstarr/5dreamrnn/1deepstarr/100random_proposal/100random_acquisition/100000cand_20000acq/init_prop_random_acq_random_20k/val_genomic/idx0/summary.json
```
...
Each round includes metrics for all test sets:
...
```bash
# All test set correlations for a round
jq '.round_3.evaluation | to_entries | map({type: .key, correlation: .value.avg_correlation})' \
  results/.../idx0/summary.json

# Compare across test sets
jq '{
  no_shift: .round_3.evaluation.no_shift.avg_correlation,
  low_shift: .round_3.evaluation.low_shift.avg_correlation,
  high_shift: .round_3.evaluation.high_shift_low_activity.avg_correlation
}' results/.../idx0/summary.json
```

### Adding New Dataset Types

**1. Create Generator** in `code/active_learning/dataset_generators.py`:

```python
class MediumShiftGenerator(DatasetGenerator):
    def __init__(self, seed: int = 42, mutation_rate: float = 0.15):
        super().__init__(seed)
        self.mutation_rate = mutation_rate
    
    def generate(self, size: int, genomic_sequences: List[str] = None, **kwargs) -> List[str]:
        return [self._mutate_sequence(seq, self.mutation_rate) 
...
DATASET_GENERATORS = {
    ...
    'medium_shift': MediumShiftGenerator,  # Add new type
}
```

**3. Define in Config** (`configs/test_val_datasets.yaml`):

```yaml
dataset_types:
  medium_shift:
    description: "Genomic sequences with 15% per-position mutation"
    mutation_rate: 0.15
    oracle_labeled: true
```
...
python scripts/train_ensemble.py \
  --model_type dream_rnn \
  --train_data data/processed/my_dataset_train.txt \
  --val_data data/processed/my_dataset_val.txt \
  --output_dir models/my_dataset_oracle \
  --n_models 5
```
...
```json
{
  "data": {
    ...
    "initial_data_path": "data/processed/my_dataset_train.txt",
    "genomic_test_path": "data/processed/my_dataset_test.txt",
    "genomic_val_path": "data/processed/my_dataset_val.txt"
  },
  "oracle": {
    "composition": [{
      "type": "dream_rnn",
      "count": 5,
      "model_dir": "models/my_dataset_oracle"
    }]
  }
}
```

**4. Generate Test/Val Datasets:**

```bash
python scripts/generate_test_val_datasets.py \
  --dataset mydataset \
  --genomic-test data/processed/my_dataset_test.txt \
  --genomic-val data/processed/my_dataset_val.txt \
  --oracle-dir models/my_dataset_oracle \
  --seed 42
```
